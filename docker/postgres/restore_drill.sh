...
DB_USER="stockops"
DRILL_DB="stockops_drill_test"

docker exec -e PGPASSWORD=stockops "$DB_CONTAINER" psql -U "$DB_USER" -d postgres -c "DROP DATABASE IF EXISTS ${DRILL_DB} WITH (FORCE);"
docker exec -e PGPASSWORD=stockops "$DB_CONTAINER" psql -U "$DB_USER" -d postgres -c "CREATE DATABASE ${DRILL_DB} OWNER ${DB_USER};"

# Step 3: Restore to test database
echo "3. Restoring to drill database..."
gunzip -c "$LATEST_BACKUP" | docker exec -i "$DB_CONTAINER" psql -U "$DB_USER" -d "$DRILL_DB"

# Step 4: Validate
echo "4. Validating restored data..."
COUNT=$(docker exec -e PGPASSWORD=stockops "$DB_CONTAINER" psql -U "$DB_USER" -d "$DRILL_DB" -t -c "SELECT count(*) FROM information_schema.tables WHERE table_schema='public';")
