...
- Name: invoices.spec.ts >> Invoices (Fatura Yönetimi) >> 'New Invoice' / 'Yeni Fatura' butonu görünür
- Location: e2e\invoices.spec.ts:125:7
...
Locator: locator('a[href*="new"], button:has-text("New"), button:has-text("Yeni"), a:has-text("Create")').first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('a[href*="new"], button:has-text("New"), button:has-text("Yeni"), a:has-text("Create")').first()
...
```ts
  ...
  126 |     const addBtn = page.locator(
  127 |       'a[href*="new"], button:has-text("New"), button:has-text("Yeni"), a:has-text("Create")'
  128 |     );
> 129 |     await expect(addBtn.first()).toBeVisible({ timeout: 8_000 });
      |                                  ^ Error: expect(locator).toBeVisible() failed
  130 |   });
  131 | });
  132 | 
