...
- Name: products.spec.ts >> Products (Ürün Yönetimi) >> ürün oluşturma formunda 'sku' alanı var
- Location: e2e\products.spec.ts:79:7
...
Locator: locator('input[name="sku"], input[placeholder*="sku" i], input[placeholder*="SKU" i]').first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('input[name="sku"], input[placeholder*="sku" i], input[placeholder*="SKU" i]').first()
