...
- Name: customers.spec.ts >> Customers (Müşteri Yönetimi) >> müşteri oluşturma formunda 'email' alanı var
- Location: e2e\customers.spec.ts:57:7
...
Locator: locator('input[name="email"], input[type="email"], input[placeholder*="email" i]').first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('input[name="email"], input[type="email"], input[placeholder*="email" i]').first()
